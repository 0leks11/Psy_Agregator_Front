...
import { PencilIcon, TrashIcon, EllipsisVerticalIcon, ArrowDownIcon, ArrowUpIcon } from '@heroicons/react/24/outline';
...
const PublicationItem: React.FC<PublicationItemProps> = ({
    ...
    const handleDelete = async () => {
        if (window.confirm('Вы уверены, что хотите удалить эту публикацию?')) {
            ...
            try {
                ...
                const errMsg = err.response?.data?.detail || "Не удалось удалить публикацию.";
    ...
    const formattedDate = new Date(publication.created_at).toLocaleDateString('ru-RU', {
        year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit'
    });

    return (
        <div className="p-6 relative group/item"> {/* Добавляем group/item для вложенного hover */}
            {isEditing ? (
                ...
                <>
                    ...
                    <div className="flex justify-between items-start mb-3">
                        <div>
                             {publication.title && <h4 className="text-lg font-semibold text-gray-800 mb-1">{publication.title}</h4>}
                        ...
                        {isEditable && (
                            <div className="absolute top-4 right-4 flex space-x-1 opacity-0 group-hover/item:opacity-100 focus-within:opacity-100 transition-opacity duration-200">
                                ...
                                <Menu as="div" className="relative inline-block text-left">
                                    <Menu.Button className="p-1 text-gray-400 hover:text-red-600" aria-label="Опции публикации">
                                            ...
                                            <div className="px-1 py-1">
                                                <Menu.Item>
                                                    {({ active }) => (
                                                        ...
                                                        >
                                                            {isDeleting ? <LoadingSpinner /> : <TrashIcon className="mr-2 h-5 w-5" />}
                                                            Удалить
                                                        </button>
                                                    )}
                                                </Menu.Item>
                    ...
                    <div
                        ...
                        className={`prose prose-sm max-w-none text-gray-700 overflow-hidden transition-all duration-300 ease-in-out ${
                            !isExpanded ? `max-h-[${MAX_COLLAPSED_LINES * LINE_HEIGHT_APPROX}rem]` : '' // Ограничиваем высоту Tailwind'ом
                        }`}
